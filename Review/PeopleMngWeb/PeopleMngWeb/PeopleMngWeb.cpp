...
BOOL WINAPI GetExtensionVersion(HSE_VERSION_INFO* pVer)
{
	...
	strcpy_s(pVer->lpszExtensionDesc, HSE_MAX_EXT_DLL_NAME_LEN, "PeopleMngWeb ISAPI Extension");
...
void SelectWindow(const wchar_t* windowID, Web::HttpConnector& h)
{
	...
	else if(wcscmp(windowID, L"ClientVwPage") == 0) {ClientVwPage page; page.Run(h);}
	else if(wcscmp(windowID, L"EmployeeVwPage") == 0) {EmployeeVwPage page; page.Run(h);}
	else if(wcscmp(windowID, L"ClientPage") == 0) {ClientPage page; page.Run(h);}
	else if(wcscmp(windowID, L"EmployeePage") == 0) {EmployeePage page; page.Run(h);}
}
