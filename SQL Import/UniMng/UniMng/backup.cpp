...
void ScheduleDlg::Window_Open(Win::Event& e)
{
	...
	try
	{
		...
		conn.ExecuteSelect(L"SELECT professor_id, professor_id FROM professor", 100, ddProfessor);
		conn.ExecuteSelect(L"SELECT course_id, professor_id FROM course", 100, ddCourse);
		conn.ExecuteSelect(L"SELECT period_id, professor_id FROM period", 100, ddPeriod);
		conn.ExecuteSelect(L"SELECT time_block_id, professor_id FROM time_block", 100, ddTime_block);
		conn.ExecuteSelect(L"SELECT classroom_id, professor_id FROM classroom", 100, ddClassroom);
